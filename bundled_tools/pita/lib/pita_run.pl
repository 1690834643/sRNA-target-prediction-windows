...
{
	dsystem ("cat $utr_fn | perl -I . lib/dos2unix.pl | perl -I . lib/fasta2stab.pl > tmp_utr_stab_$r");
	dsystem ("cat $mir_fn | perl -I . lib/dos2unix.pl | perl -I . lib/fasta2stab.pl > tmp_mir_stab_$r");
}
...
if ($upstream_fn eq "")
{
	dsystem ("cat tmp_utr_stab_$r " .
			 "| cut -f 1 " .
			 "| perl -I . lib/add_column.pl -s $POLY_A " .
			 "> tmp_prefix_$r;");
}
else
{
	...
	{
		dsystem ("cat $upstream_fn | perl -I . lib/fasta2stab.pl > tmp_upstream_stab_$r");
	}
	
	dsystem ("cat tmp_utr_stab_$r " .
			 "| cut -f 1 " .
			 "| perl -I . lib/add_column.pl -s $POLY_A " .
			 "| perl -I . lib/join.pl -o A - tmp_upstream_stab_$r " .
			 "| perl -I . lib/merge_columns.pl -1 1 -2 2 " .
			 "> tmp_ext_utr_$r;");
	
	dsystem ("cat tmp_ext_utr_$r " .
			 "| perl -I . lib/stab2length.pl " .
			 "| perl -I . lib/cut.pl -f 1,1,2,2 " .
			 "| perl -I . lib/modify_column.pl -c 2 -s " . ($CDS_FLANK-1) . " " .
			 ">tmp_ext_utr_pos_$r");
			 
	dsystem ("cat tmp_ext_utr_$r " .
			 "| perl -I . lib/extract_sequence.pl -f tmp_ext_utr_pos_$r " .
			 "> tmp_prefix_$r");
}

my $ext_utr_fn = $output_dir . $prefix . "ext_utr.stab";

dsystem ("perl -I . lib/join.pl tmp_prefix_$r tmp_utr_stab_$r " .
         "| perl -I . lib/add_column.pl -s $POLY_A " .
         "| perl -I . lib/merge_columns.pl -1 1 -2 2 " .
         "| perl -I . lib/merge_columns.pl -1 1 -2 2 " .
         "> " . $ext_utr_fn);
...
dsystem ("perl -I . lib/find_potential_mirna_targets.pl tmp_utr_stab_$r -f tmp_mir_stab_$r $echoed_pt_args > tmp_pt_$r");

# Count lines in tmp_pt file using pure Perl (portable, no shell wc/sed/cut needed)
my $n_potential_targets = 0;
if (open(my $pt_fh, '<', "tmp_pt_$r")) {
    while (<$pt_fh>) { $n_potential_targets++ }
    close($pt_fh);
}
if (length($limit) > 0 and $n_potential_targets > $limit)
{
   	dsystem ("rm -rf tmp_prefix_$r tmp_ext_utr_$r tmp_ext_utr_pos_$r tmp_prefix_$r tmp_pt_$r tmp_utr_stab_$r tmp_mir_stab_$r tmp_upstream_stab_$r");
	exit(1);
}
...
dsystem ("cat tmp_pt_$r " .
		 "| perl -I . lib/join.pl -1 2 - $ext_utr_fn " .
		 "| perl -I . lib/cut.pl -f 1-9,11- " .
		 "| perl -I . lib/modify_column.pl -c 2,3 -a $CDS_FLANK " .
		 "| perl -I . lib/RNAddG_compute.pl -ddgarea $DDG_AREA -upstream_rest $FLANK_UP -downstream_rest $FLANK_DOWN " .
		 "| perl -I . lib/modify_column.pl -c 2,3 -s $CDS_FLANK " .
		 "| perl -I . lib/modify_column.pl -c 9,10,11,12,13,14 -m '\"-1\"' " .
		 "| perl -I . lib/cut.pl -f 1-15,14 " .
		 "| perl -I . lib/modify_column.pl -c 15 -sc 14 " .
		 "| perl -I . lib/cut.pl -f 1-12,14-16,13 " .
		 "| perl -I . lib/modify_column.pl -c 9,10,11,12,13,14,15 -p 2 -m '\"-1\"' " .
		 "| perl -I . lib/cap.pl \"UTR,microRNA,Start,End,Seed,Mismatchs,G:U,Loop,Site size,dGduplex,dG5,dG3,dG0,dG1,dGopen,ddG\" " .
...
sub dsystem {
        ...
        if ($^O eq 'MSWin32') {
                # On Windows, replace POSIX ; separator with cmd.exe & separator
                # Keep pipes (|) and redirects (>) as-is - cmd.exe handles these
                # But be careful: don't replace ; inside quoted strings
                (my $win_cmd = $cmd) =~ s/;/ \&/g;
                system($win_cmd);
        } else {
                system ($cmd);
