...
my $RNAHYBRID_EXE_DIR = ".";
my $RNAddG_EXE_DIR 		= ".";

# Per-process temp file names so concurrent PITA chunks running in the same
# bundle CWD do not clobber each other's RNAduplex/RNAddG input. $$ is the
# perl process pid, which is unique across the scheduler's worker pool.
my $TMP_SEQFILE1 = "tmp_seqfile1_$$";
my $TMP_SEQFILE2 = "tmp_seqfile2_$$";
...
print STDERR " Done.\n";
unlink $TMP_SEQFILE1 if -e $TMP_SEQFILE1;
unlink $TMP_SEQFILE2 if -e $TMP_SEQFILE2;
...
sub getdGduplexes {
	...
	open (SEQFILE, ">$TMP_SEQFILE1") or die ("Could not open temporary sequence file '$TMP_SEQFILE1'.\n");
	...
	my $cmd = "RNAduplex.exe -5 0 < $TMP_SEQFILE1";
	...
	{
		# RNAduplex custom binary not available - return placeholder values
		# This allows the pipeline to complete with -1 for dG values
		print STDERR " (RNAduplex unavailable, using placeholder values)\n";
		for (my $i = 0; $i < $insize; $i++) {
			push (@outArray, join ("\t", 0, "-1", "-1", "-1"));
		}
		return @outArray;
	}
...
sub getddG {
	...
	open (SEQFILE, ">$TMP_SEQFILE2") or die ("Could not open temporary sequence file '$TMP_SEQFILE2'.\n");
	...
	my $cmd = "RNAddG4 -u $upstream_rest -s $bindEnd -f $target_len -t $target_len < $TMP_SEQFILE2";

	print STDERR "Calling RNAddG with " . @inArray . " targets... ";

	my @resArray = split (/\n/, `$cmd`);

	if (scalar(@resArray) == 0) {
		# RNAddG4 binary not available - return placeholder values
		print STDERR " (RNAddG4 unavailable, using placeholder values)\n";
		for (my $i = 0; $i < scalar(@inArray); $i++) {
			push (@outArray, join ("\t", "-1", "-1", "-1"));
		}
		return @outArray;
	}
...
# 	
# 	my $resline = `RNAduplex.exe < tmp_seqfile1`;
# 	chomp ($resline);
# 	
