...
my $runcmd = "perl -I . lib/pita_run.pl -utr $utr_fn -mir $mir_fn -prefix ${prefix}_ " .
...
win_system("rm ${prefix}_ext_utr.stab; mv ${prefix}_pita_results.tab tmp_${pid}; head -n 1 tmp_${pid} | cut -f 1-5,8,10- > ${prefix}_pita_results.tab; cat tmp_${pid} | perl -I . lib/body.pl 2 -1 | tr -d '\r' | cut -f 1-8,10- | perl -I . lib/merge_columns.pl -1 4 -2 5 -d ':' | perl -I . lib/merge_columns.pl -1 4 -2 5 -d ':' | sed 's/Seed:Mismatchs:G:U/Seed/g' | perl bin/sort_wrapper.pl -k 13n >> ${prefix}_pita_results.tab; rm tmp_${pid};");

win_system("cat ${prefix}_pita_results.tab | perl -I . lib/body.pl 2 -1 | cut -f 1,2,13 | perl -I . lib/modify_column.pl -c 2 -m '\"-1\"' | perl -I . lib/average_rows.pl -k 0,1 -losoe -n | perl -I . lib/cut.pl -f 2,3,1,4 | perl -I . lib/modify_column.pl -c 3 -m '\"-1\"' | perl -I . lib/modify_column.pl -c 3 -p 2 | perl bin/sort_wrapper.pl -k 4n | perl -I . lib/cap.pl \"RefSeq,microRNA,Sites,Score\" > ${prefix}_pita_results_targets.tab");
...
if ($gxp == 1)
{
   ...
   win_system("lib/fasta2stab.pl $utr_fn | perl -I . lib/stab2length.pl | perl -I . lib/lin.pl | perl -I . lib/add_column.pl -s 0 | perl -I . lib/cut.pl -f 2,1,4,3 | perl -I . lib/add_column.pl -s 0 | perl -I . lib/add_column.pl -s 1 | perl -I . lib/tab2feature_gxt.pl -n 'UTR Sequences' >> ${prefix}_pita_results.gxp");
   win_system("perl -I . lib/body.pl 2 -1 ${prefix}_pita_results.tab | tr -d '\r' | perl -I . lib/cut.pl -f 1-4,2,14 | perl -I . lib/uniquify.pl -c 1 | perl -I . lib/tab2feature_gxt.pl -n 'PITA Predictions' -c '0,0,255,1'  -lh 50 -l 'Filled box' -minc '0' -maxc '25' >> ${prefix}_pita_results.gxp");
